...
public class UIUtil {
    ...
    public synchronized static void setDrawableTop(TextView v, int drawableId) {
        if (v == null) return;
    ...
    public synchronized static void setDrawableLeft(TextView v, int drawableId) {
        if (v == null) return;
    ...
    public synchronized static void setDrawableRight(TextView v, int drawableId) {
        if (v == null) return;
    ...
    public static int getStatusHeight() {
        ...
        if (resourceId > 0) {
            mStatusHeight = BaseApplication.getContext()
                    .getResources()
     ...
     * 高亮
     */
    public synchronized static void textHighlight(TextView textView, String string, String mark) {
        if (TextUtils.isEmpty(string) || textView == null) return;
        ...
        styled = new SpannableStringBuilder(string);

        Pattern p = Pattern.compile("[" + mark + "]");
        Matcher m = p.matcher(newString);
        while (m.find()) {
            int start = m.start();
            styled.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.libCommonc_4285F4)), start, start + 1,
     ...
     * 高亮
     */
    public synchronized static void textHighlight(TextView textView, String string, String mark, String id) {
