...
public final class StorageUtil {
     ...
     */
    @SuppressWarnings("TryWithIdenticalCatches") private static File getCacheDirectory(boolean preferExternal) {
        ...
        if (appCacheDir == null) {
            appCacheDir = UIUtil.getContext()
                    .getCacheDir();
        }
        if (appCacheDir == null) { // 已过时，api8 以下需要自己创建缓存目录
            //String cacheDirPath = "/data/data/" + UIUtil.getContext()
            //        .getPackageName() + "/cache/";
            String cacheDirPath = UIUtil.getContext()
                    .getCacheDir().getPath();
    ...
    private static File getExternalCacheDir() {
        File externalCacheDir = UIUtil.getContext()
                .getExternalCacheDir();
        ...
        File appCacheDir = new File(new File(dataDir, UIUtil.getContext()
                .getPackageName()), "cache");
    ...
    public static File getPublicDirectory(String fileName) {
        ...
        if (appCacheDir == null) {
            appCacheDir = getUnderCacheDirectory(fileName, true);
        }
    ...
    private static boolean hasExternalStoragePermission() {
        int perm = ContextCompat.checkSelfPermission(UIUtil.getContext(), EXTERNAL_STORAGE_PERMISSION);
